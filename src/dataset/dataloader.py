...
print(f'[dataloader]  {sys.path=}')

from src.dataset.utils import cvtColor, preprocess_input
...
class CenternetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # img_ = image.copy()
        # img_ = CVDraw.rectangles(img_, BBoxes(box, mode='xyxy'))
        # CVDraw.Show().show(img_, wait_time=-1)
...
if __name__ == '__main__':
    ...
    batch_size      = 2
    num_workers     = 4
    annotation_path   = '/data/ylw/code/pl_yolo/git/centernet-pytorch/2007_val.txt'
    ...
    ds = CenternetDataset(lines, input_shape, num_classes, train = True)
    ...
    loader = DataLoader(
        ...
        pin_memory=True,
        drop_last=True, 
        collate_fn=ds.dataset_collate
    )
    
    for batch_idx, batch in enumerate(loader):
        print(f'[{batch_idx}]  {batch[0].shape=}  {batch[1].shape=}  {batch[2].shape=}  {batch[3].shape=}  {batch[4].shape=}')
        
        mask = batch[4]
        expand_mask = torch.unsqueeze(mask,-1).repeat(1,1,1,2)
        print(f'{expand_mask.shape=}')
        m = mask.numpy()
        m = np.hstack((m[0], m[1]))
        plt.imshow(m)

        m = expand_mask.numpy()
        m1 = np.hstack((m[0, :, :, 0], m[0, :, :, 1]))
        m2 = np.hstack((m[1, :, :, 0], m[1, :, :, 1]))
        m = np.vstack((m1, m2))
        plt.imshow(m)
